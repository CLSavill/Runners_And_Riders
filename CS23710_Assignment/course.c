...
int get_number_of_courses(char* file_name, int* number_of_courses_ptr) {
    ...
    char line_input[40]; // Size 18 used just incase a large number of courses are contained in a course.
    ...
    while (fgets(line_input, sizeof (line_input), courses_file)) { // While end of file has not been reached.
        (*number_of_courses_ptr)++; // Counts the number of courses.
    }

    fclose(courses_file); // Closes file as no longer needed.
    return (*number_of_courses_ptr);
}
///////////////////////////////////////////////////////////////////////////
...
int courses_file_load(event_ptr event, char* file_name, int* number_of_courses_ptr) {
    ...
    for (counter; counter < (*number_of_courses_ptr); counter++) {
